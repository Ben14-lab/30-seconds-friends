...
// ---------- remember who I am on this device (survives closing the app) ----------
const LS_PLAYER_ID = "tsg-player-id";
const LS_LAST_ROOM = "tsg-last-room";
function lsGet(key) {
  try {
...
function lsSet(key, value) {
  try {
    ...
    // storage not available (private mode etc.) — app still works, just forgets you
  }
}
function getOrCreateMyId() {
  const saved = lsGet(LS_PLAYER_ID);
...
export default function App() {
  const [myId] = useState(getOrCreateMyId);
  const [lastRoom] = useState(() => lsGet(LS_LAST_ROOM));
  ...
  const joinExistingRoom = async (codeOverride) => {
    const trimmed = (typeof codeOverride === "string" ? codeOverride : joinCodeDraft).trim().toUpperCase();
    if (!trimmed) return;
  ...
  const joinSession = async (name) => {
    ...
    if (!trimmed) return;
    const next = await updateRoom(roomCode, (latest) => {
      // Coming back? If a player with the same name is already in this room (e.g. you
      // switched phone/browser), take over that spot: same team, and host if you were host.
      const existing =
        latest.players.find((p) => p.id === myId) || latest.players.find((p) => sameName(p.name, trimmed));
      const oldId = existing ? existing.id : null;
      const swap = (id) => (oldId && id === oldId ? myId : id);

      const players = existing
        ? latest.players.map((p) => (p.id === oldId ? { ...p, id: myId, name: trimmed } : p))
        : [...latest.players, { id: myId, name: trimmed, teamIdx: null }];

      const hostId = swap(latest.hostId);
      // Become host if: I created this room, I already was host, OR there is no host yet and nobody else is here.
      const noHostYet = !hostId;
      const becomeHost =
        iCreatedRoomRef.current || hostId === myId || (noHostYet && latest.players.length === 0);
      ...
      return {
        ...
        players,
        teams,
        hostId: becomeHost ? myId : hostId,
        designatedExplainerId: swap(latest.designatedExplainerId),
        explainerId: swap(latest.explainerId),
      };
    });
    if (next) setSession(next);
  };
...
/* ----------------------------- ROOM ENTRY ----------------------------- */
function RoomEntryScreen({ joinCodeDraft, setJoinCodeDraft, onCreate, onJoin, lastRoom }) {
  return (
    <div style={S.centerWrap}>
      ...
      {lastRoom && (
        <>
          <button style={S.startBtn} className="sg-btn" onClick={() => onJoin(lastRoom)}>
            <Users size={18} /> Terug naar room {lastRoom}
          </button>
          <div style={S.orDivider}>of</div>
        </>
      )}

      <button style={lastRoom ? S.primaryGhostBtn : S.startBtn} className="sg-btn" onClick={onCreate}>
  ...
  return (
    <div style={S.centerWrap}>
      ...
      <p style={S.helperText}>
        Deed je al mee (bijv. op een ander apparaat)? Vul dan precies dezelfde naam in — je krijgt
        je oude plek terug, inclusief team en host-rol.
      </p>
    </div>
  );
}
